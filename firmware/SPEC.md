...
| **退避** | **記録を始めるとき、空きが 1MB を切っていたら自動生成の古いログから消す**（`auto*` と `r*` だけ。名前順＝生成順）。消したものは記録の先頭に `note` で残す |
...
| | |
|---|---|
| 記録を始めるとき | 空きが **1MB** を切っていたら、**自動生成の古いログから消す**。対象は `auto*` と `r*` だけで、手で名付けた探索ログ（`probe.bin` など）は触らない。名前順が生成順と一致する（連番と日時）。消したものは記録の先頭に `note` で残す |
| それでも書けないとき | **`write()` の戻り値を見て記録を止める。** 静かに落とさない |
