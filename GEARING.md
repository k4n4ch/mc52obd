...
1. **最高速はほぼ変わらない**（133 → 132 km/h）。このクラスは出力律速であり、ギア比が
   届く届かないの問題ではない。
2. **6 速の余力は全速度域で減る**。100 km/h で 6.2 → 5.0 kW（−19%）、120 km/h で
   3.9 → 2.5 kW（−37%）。速度が上がるほど差が開く。
3. **6 速 1 枚で走れる車速範囲が 60〜120 km/h から 64〜110 km/h へ縮む**（幅で 23% 減）。
4. **走行性能そのものは失われていない**。15T の 5 速は 14T の 6 速より 4.8% ショートで、
   14T の 6 速がこなしていた仕事をすべて引き受ける。変わったのは段の役割分担であり、
   到達できる性能ではない。
5. **高速道路の巡航は成立する**。100 km/h は 6 速で余力 5.0 kW、勾配 8.3% 相当まで維持。
   120 km/h 区間は平坦・無風なら 6 速で維持できるが余力は 14%（使用率 86%）。
...
## 15T 5 速は 14T 6 速の置き換えになる

15T 5 速の総減速比 7.855 は 14T 6 速の 7.492 より 4.8% ショート。全速度域で余力が上回り、
実用窓の上端も 123 km/h と 14T 6 速の 120 km/h を超える。

したがって **14T の 6 速でこなしていた仕事は、15T では 5 速がそのまま引き受ける**。
従来の 1〜6 速の守備範囲が 1〜5 速に収まり、6 速がその上に載る巡航専用のオーバードライブ
として増えた、という構造になる。到達できる性能は失われていない。

失われたのは「6 速に入れっぱなしで 60 から 120 まで済む」という運用上の単純さである。
得られたのは 100 km/h 巡航で 6,497 → 6,064 rpm（**−433 rpm**）の回転低下で、単気筒では
振動と騒音に直接効く。
...
到達できる性能は変わらないため、選択は運用上の好みに落ちる。

- **14T** — 6 速 1 枚で 60〜120 km/h をカバーする単純さ。高速域でシフトを要求されない。
- **15T** — 常用域の回転が 6.7% 下がり、巡航の振動と騒音が減る。ただし追い越し・登坂・
  向かい風では 5 速を使う前提を受け入れる必要がある。

**15T のまま 6 速を万能ギアとして使い続けようとすると不満が出る。** 120 km/h で使用率 86%、
向かい風 6 m/s で上限 108 km/h まで落ちるため。5 速へ落とす運用を許容できるかが分岐点になる。
