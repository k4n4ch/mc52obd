...
実車の 12V ラインは変動が大きい。**ただし、こちらが持っている電圧データは PID `42` を
約 1.3Hz でポーリングしたもので、過渡現象は原理的に捉えられない**（観測された最大
16.2V も過渡の測定ではない）。実際の過渡環境は未測定という前提で設計している。
...
| **常用リップル** | TVS の立ち上がり 18V は観測最大 16.2V の上。これを超える環境なら**車両側の電装が先に壊れる**ので、そこを守る設計にする意味は薄い |
