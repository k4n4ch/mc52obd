...
`0b1001` `0b1101` が加わり 4 値になった。**bit 2 と bit 3 はいずれもスロットルに
従う。** 単一変数のしきい値で分離能を測ると、bit 2 はスロットル生 `[6] ≥ 29` で
93.4%（全閉は 26）、bit 3 は `[6] ≥ 29` かつ車速 ≥ 20 で 94.9%。回転数を揃えて
比べると、どの帯でも bit 3 = 1 の側がスロットル・噴射・MAP のすべてで高い
（3,500–4,000rpm でスロットル 27 → 36、噴射 405 → 616、MAP 47 → 70）。

当初は bit 3 を「約 15km/h のしきい値」、bit 2 を「2,500rpm のしきい値」と読んだが、
**いずれも交絡による見間違いであった。** 4 つのビット組み合わせごとに中央値を並べた
だけで、回転数・車速・負荷を分けていなかった。市街地では速度と負荷が一緒に動くため、
負荷の旗が速度のしきい値に見える。**93〜95% なので推定であり、同定ではない。**
...
| 7 | スロットル（`[6]` の派生） | `0.770 × [6] − 20.08` | **同定**（全閉基準の一次関係）。**スケール未確定** |
...
| 8 | 状態ビットフィールド | bit 2 ≒ スロットル開（`[6] ≥ 29`、全閉 26）93.4% / bit 3 ≒ スロットル開＋走行中 94.9% | **推定** |
| 5 6 7 | 不明 | 全 11,135 フレームで `00`。ただし**高回転・半開超・冷間始動・故障状態は未踏**なので「未使用」とは言えない | **未同定** |
| 9 | **ラジエータファン** | 入 100℃ / 切 98℃（99℃ は履歴依存）。走行中は 96℃ までしか上がらず一度も立たない | **同定** |
...
**未同定のフィールドが残る。** `0xD1` index 8 の bit 2、同 index 5/6/7、テーブル `0x20` と
`0xD0` の中身。`0x11`（センサ群）と `0xD1`（駆動状態）をアンカーとして同時に読む
必要があり、4 本同時でも各 5.0Hz を維持できる（応答するテーブル 1 本のコストは
37ms で、周期 200ms に 5 本まで入る）。
