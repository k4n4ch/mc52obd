...
| 6 | スロットル位置（絶対） | `A × 100/255` [%] | **同定**（標準層 `11` と一致） |
| 7 | スロットル位置（相対） | `A × 100/255` [%] | **同定**（標準層 `45` 相当） |
| 8-9 | 冷却水温 | `[9] − 40` [℃] | **同定** |
| 10-11 | 吸気温 | `[11] − 40` [℃] | **同定** |
| 12-13 | 吸気管絶対圧 | `[13]` [kPa] | **同定**（単気筒の脈動をそのまま返す） |
| 16 | 制御モジュール電圧 | `[16] / 10` [V] | **同定** |
| 17 | 車速 | `A` [km/h]（切り捨て） | **同定**（標準層 `0D` と同じ信号、§4.8） |
| 18-19 | **噴射時間** | BE u16、範囲 0〜1546 | **同定**（量）。**単位・スケールは未確定** |
