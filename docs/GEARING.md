...
**CdA は実測で検証できていない。** ただし **2026-09-19 以降は測る手段がある** ——
下の「コーストダウンで CdA と Crr を実測できる」。
...
### コーストダウンで CdA と Crr を実測できる（2026-09-19 に手段が揃った）

**トルクを測る代わりに抵抗を測る。** Track B の基板で独自層を叩くと、標準 OBD には
無い 2 つが取れる（[`FINDINGS.md`](FINDINGS.md)）。

| | 意味 |
|---|---|
| 噴射時間 `0x11[18-19]` | **`0` なら燃料カット。** エンジンが純粋な制動要素になっている |
| 駆動状態 `0xD1[4]` | `00` 駆動 / `01` N かクラッチ切り |

この 2 つで**惰行を 2 種類に分けられる。**

| 条件 | 減速に効いている力 |
|---|---|
| `index 4 = 01` の惰行 | **空気抵抗 ＋ 転がり抵抗のみ** |
| 噴射 = 0 かつ `index 4 = 00` | 空気 ＋ 転がり ＋ **エンジンブレーキ** |
| **差** | **エンジンブレーキ**。段ごとに取れば回転数依存も出る |
...
を当てれば、**`v²` 項と定数項として CdA と Crr が分離して求まる。** 上の訂正で
「スロットル開度から抗力の形を確かめる」筋が否定されたが、**速度の減衰そのものを見れば
スロットルの非線形性を経由しない。**

**必要なのは高速側の長い惰行。** 手元の走行 2 本（2026-09-19、市街地）にはカットが
167 サンプル入っているが、減速区間が短く `v²` 項が効かないので分離できない。
**60km/h 以上から数十秒かけて自然に落ちる区間**を、勾配の小さい道で数本取る。
...
実走ログ（フロント 15T 装着車、5 走行）から 6 速の定常点を抽出し、計算値と比較した。
定常の判定は前後 2 サンプル（約 4 秒）で OBD 車速の変動が 2 km/h 以内。**実車速は
`(0D + 0.5) × 15/14`。** 車速センサはファイナル上流にあり、ECU は 14T を前提に換算するので、
OBD 値は実車速より丁数比ぶん低く出る。`0D` は切り捨てなので真値は +0.5（FINDINGS.md 参照）。
ギヤ判定もアプリと同じ規則（`k` = 1.0057、窓 ±4% ＋ 量子化 `0.5/車速`、8 km/h・1,700rpm 下限）
に揃えてある。

| 実車速 | n | 実測 rpm | 計算 rpm | 差 | スロットル |
|---:|---:|---:|---:|---:|---:|
| 35–40 km/h | 4 | 2332 | 2274 | +2.5% | 11.6% |
| 40–45 | 6 | 2536 | 2577 | −1.6% | 11.0% |
| 45–50 | 26 | 2968 | 2880 | +3.1% | 14.9% |
| 50–55 | 82 | 3234 | 3183 | +1.6% | 15.3% |
| 55–60 | 187 | 3497 | 3487 | +0.3% | 16.5% |
| 60–65 | 156 | 3858 | 3790 | +1.8% | 18.4% |
| 65–70 | 71 | 4090 | 4093 | −0.1% | 17.6% |
| 70–75 | 37 | 4297 | 4396 | −2.3% | 16.5% |
| 75–80 | 14 | 4734 | 4699 | +0.7% | 20.0% |
| 80–85 | 39 | 5083 | 5003 | +1.6% | 25.5% |
| 85–90 | 123 | 5335 | 5306 | +0.6% | 32.5% |
| 90–95 | 103 | 5693 | 5609 | +1.5% | 34.5% |
| 95–100 | 82 | 5954 | 5912 | +0.7% | 35.1% |
| 100–105 | 88 | 6222 | 6215 | +0.1% | 36.7% |
| **105–110** | 29 | **6493** | 6519 | **−0.4%** | 38.4% |

n 計 1,047 点。中央値 +0.70%、n で重み付けた平均 +0.78%、範囲 −2.3〜+3.1%。

**残差に意味のある速度依存は無い。** 重み付き回帰の傾きは −0.012 %/(km/h) で、35 → 110 km/h
の全域を通しても −0.85pt にしかならず、バケツ間のばらつき（±2.5pt）に埋もれる。ギアリングの
計算部分は 35〜110 km/h の全域で裏付けられている。

残差が正へ偏るのは、本書の `R(n)` が**幾何円周 1.9222 m** で計算しているのに対し、ECU が
前提とする円周がそれより小さいため。**残差 +0.78% から逆算すると 1.9074 m** で、`k` = 1.0057
から独立に得た **1.911 m** と 0.2% で一致する（FINDINGS.md）。**ギア比の連鎖そのものが正しい**
ことも別途確認されている（段ごとに `k` を独立に当ててばらつき 0.13%）。

**当初この突合は 92 km/h 止まりで、100 km/h 以上は計算のみだった。** 2026-08-30 の 3 走行
（13,153 行）で 110 km/h までの定常点が得られ、その留保は外れた。旧 `k` = 1.0148 と
`÷0.935` で作った版から、切り捨てを織り込んだ現行の定数で作り直してある。

### 6 速の高回転域まで予測が当たる（2026-09-05）

上表より上の領域、**6 速で回転数が 8,000rpm に達する点**でも実測が取れた。

- 諸元から計算した車速に対し、**実測は −1.5%**
- 同じ点で回転数から見ると、**計算に対し実測は +1.0〜1.5%**

ずれの符号と大きさは、上で述べた **+0.78%（ECU 想定円周が幾何円周より小さい）** と
同じ向きで整合する。**ギアリングの計算は 6 速の上端まで有効**と言える。

この点でのモデル上のトルク使用率は **98%**、余裕は 0.4kW を切る。本書が「15T の
最高速は出力律速」としていた領域に実測が乗った形になる。

**同時に、スロットル開度が負荷の代理変数にならないことの傍証も得られた。** この点の
スロットルは約 50% しかない。高回転では吸気がスロットルではなく回転数で律速されるため、
開度が半分でもトルクはほぼ使い切っている。
...
**PID 04（Calculated Load）。** 実車速 55 → 95 km/h の定常走行で 52 → 77% にしか動かない。
所要出力はこの間に 4.0 倍（1.94 → 7.75 kW）になるのに 1.5 倍で、追随していない。別途 13,153 行で調べたところ、**取得の鮮度を揃えると
...
ここまでは諸元と仮定から「使えるはずの範囲」を出してきた。実走 3 本（2026-08-30、
13,153 行、うちギヤ確定 10,955 点）で**実際に使った領域**を同じ平面に重ねる。

![実走行の使用密度](fig5-usage.svg)

| 段 | 点数 | 割合 | 実車速 5–95% | 回転数 5–95% | 3,900rpm 未満 |
|---:|---:|---:|---|---|---:|
| 1 | 280 | 2.6% | 11–28 | 2255–5783 | **67%** |
| 2 | 2353 | 21.5% | 21–54 | 2767–7166 | 21% |
| 3 | 2822 | 25.8% | 33–69 | 3180–6732 | 15% |
| 4 | 2656 | 24.2% | 41–77 | 3306–6039 | 31% |
| 5 | 1554 | 14.2% | 50–87 | 3424–5947 | 26% |
| 6 | 1290 | 11.8% | 51–103 | 3106–6337 | **40%** |

### 仮定と実使用のずれ

**「実用下限 3,900rpm」は使用の下限ではない。** 全体の **26%**、6 速に限れば **40%** が
3,900rpm を下回っている。実用窓の下端はこの値で決めているので、**窓は「使える範囲」であって
「使っている範囲」ではない**。

両者は別の量で、どちらも正しい。低回転で粘ること自体はできるが、そこから加速するには
下の段へ落とす必要がある。窓の下端は「そこから加速できる下限」を意味している。

**トルクピークは実質使っていない。** 8,000rpm を超えたのは全体の **0.3%**。本書が
「実用する全速度域がトルクピークの手前側にある」と述べた点は、実測でも裏付けられた。
最も回しているのは 2 速で 95 パーセンタイルが 7,166rpm、それでもピークには届いていない。

**1 速はほぼ判定できない。** 点数が 2.6% と極端に少なく、67% が 3,900rpm 未満。発進・停止
専用で大半が半クラ状態のため、回転数と車速の比が成立せず判定から落ちている。図で 1 速の帯が
薄いのはそのため。

### 6 速の実使用

**51〜103 km/h（5–95%）** を 6 速で走っている。本書の実用窓（64〜110 km/h）と比べると、
**下端は窓より 13km/h 低く、上端は窓の内側**に収まる。下端のずれは上記のとおりで、
上端が余っているのは単に 110km/h 以上を出す場面が少なかったということ。
