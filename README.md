...
- **捨てた区間は道路形状で補う** — 端点の座標と進行方位を OSRM に渡して経路を引き、**OBD 車速の
  ...
  - **合否は「経路長と車速積分の一致」で見る。** 独立に得た 2 つの量なので、一致すれば経路が
    正しいことの裏付けになる。乖離 8% 超、または含意される平均速度が 8〜140km/h の外に出たら
    **線を引かない**（黙って消さず、状態行と console に理由を出す）
  - 実測: 7 区間中 6 区間を補完（0.11〜1.08km、乖離 −0.39〜−5.68%）。残る 1 区間は OSRM が
    誤った経路を返し「1.74km を 38 秒＝平均 164km/h」として棄却された。トンネルの無い走行では
    100m 未満の空白として最初から対象外になり、線は引かれない
...
`p3sweep.py` は**停車＋アイドリングで測れる**（走行不要）。実走ログの解析で脱落率が
回転数帯・車速帯を通じて平坦（停車中も 38%）だったため、停車での測定に代表性がある。
PID 別の欠測率まで分解して出すので、スマホ側のタップ巡回より系統的に詰められる。
