...
- **自車位置は計測点の間を内挿する。** 計測は約 1.3Hz しかないので、行の位置に
  スナップさせると自車が 10〜25m 刻みで飛ぶ（ナビモードでは特に目立つ）。表示の
  基準時刻を連続値で持ち、前後の点から内挿する。**区間内の配分は車速で行う** ——
  単純な時間比だと、欠測やトンネル補完で間隔が空いたところが実際の移動と合わない
