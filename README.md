...
**「前丁数推定」で GPS から自動入力できる。** 補正係数を `c = Σ(GPS) / Σ(0D)`（和の比。
各点の比を平均すると分母のノイズで上振れする）で測り、`前 = 14 × c × 後 / 36` を解いて
丸める。GPS が信頼できる 20km/h 以上のみ使う。設定を保存していない状態で CSV を読むと
自動で入り、以後は手動で上書きできる（保存済みの設定を勝手に書き換えることはしない）。

判定は丁数の丸め残差ではなく、**丸めた組み合わせが実測の補正係数をどれだけ再現するか**で
行う（1% 超で警告）。丸め残差では判定力が足りないため —— 後 39T と誤設定すると 16T へ
丸まって残差は 0.26 と小さいが、係数は 1.6% ずれる。

**推定できるのは前丁数だけで、後丁数は実車の値が入っている前提。** 速度比から (前, 後) の
組を一意に決めることはできない（18/43 と 15/36 は比が 0.4% しか違わず区別できない）。
誤差の検査で弾けるのは、指定された後丁数と辻褄が合わない組み合わせまで。
