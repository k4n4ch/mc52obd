...
各点の比を平均すると分母のノイズで上振れする）で測り、現実的なレンジ（前 12-18T /
後 28-48T、プルダウンの選択肢と同じ定義）を総当たりして候補を並べる。GPS が信頼できる
20km/h 以上のみ使う。設定を保存していない状態で CSV を読むと自動で入り、以後は手動で
上書きできる（保存済みの設定を勝手に書き換えることはしない）。

**「最も合う」組を選んではいけない。** GPS の測定誤差が候補間の間隔より大きいため。実測でも
走行間で係数が 1.0719 と 1.0663（0.5% 差）とばらつき、あるログでは 17/41 が誤差 0.01% で
実車の 15/36（0.48%）を上回って首位に来た。**0.01% の差には意味が無い。**

そこで**測定誤差の範囲（0.6%）内は同点**とみなし、その中では純正からの変更が少ないものを
採る（前後とも替える人は少ない、という事前分布）。データだけでは決まらないので事前分布に
決めさせる、という判断を明示している。同点の候補は画面に併記して実車と照合できるようにした。

この方式で実ログ 2 本とも実車の 15/36T を当て、補正後の車速/GPS は 0.9995 / 1.0048 になる。
