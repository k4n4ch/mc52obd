...
**表示車速はスプロケット丁数で補正できる。** ログボタンで開くパネルに前後丁数を入力する。
...
**表示車速はスプロケット丁数で補正する。** ツールバーの「丁数」に前後を入れる。設定は走行画面と
`localStorage` を共有しており、どちらで変えても両方に効く。**補正するのは表示（グラフ・
カーソル値・線の太さの区分）だけで、ギヤ判定には掛けない** —— k は生値に対して定義されて
いるため。実ログで 15T を設定すると表示車速と GPS 速度の比が 1.000 になり、ギヤ確定率は
丁数を変えても 65% のまま動かないことを確認済み。

**「丁数推定」で GPS から自動入力できる。** 補正係数を `c = Σ(GPS) / Σ(0D)`（和の比。
各点の比を平均すると分母のノイズで上振れする）で測り、レンジ内を総当たりして候補を並べる。
GPS が信頼できる 20km/h 以上のみ使う。設定を保存していない状態で CSV を読むと自動で入り、
以後は手動で上書きできる（保存済みの設定を勝手に書き換えることはしない）。

### 丁数のレンジは実際の品揃えに合わせてある（2026-08 調べ）

| | 設定範囲 | 根拠 |
|---|---|---|
| 前（ドライブ） | **12-15T** | サンスター 品番361 の設定歯数が 12-15T、キタコ `530-1818` は 13/14/15T。**16T 以上は存在しない**。15T でもスプロケットカバーへの干渉が報告されるほどで物理的に大きくできない |
| 後（ドリブン） | **34-38T** | サンスター `RH-133` は **36T（純正歯数）のみ**、NTB `SPH-086R` も 36T。主要メーカーが他を出しておらず**事実上固定**。他車種流用に備えて前後 2 丁だけ余裕を持たせた |

純正は 14T/36T、520 サイズ 108L。

**このレンジだと前の丁数は 7.1% 刻みになり、GPS の測定誤差 0.5% に対して一桁の余裕がある。**
候補が競合しないので推定は安定する。実ログ 2 本とも候補 2 件に絞れ、どちらも実車の 15/36T を
一位で当てた（対抗の 14/34 は誤差 1.2% / 0.7% で明確に劣る）。補正後の車速/GPS は
0.9995 / 1.0048。

念のため**測定誤差の範囲（0.6%）内は同点**とみなし、その中では純正からの変更が少ないものを
採る処理を残してある。レンジを広く取ると候補間隔が測定誤差を下回って順位が信用できなくなる
（前 12-18T / 後 28-48T で試したときは、実車と違う 17/41 が誤差 0.01% で 15/36 の 0.48% を
上回って首位に来た）。**レンジを現実に合わせることが精度の前提**になっている。

配色について。当初はギヤを順序尺度とみて単一色相の逐次ランプを使い validator も通していたが、
**実データでは青系が並んで判別できなかった**ため、実用を優先して**6 色のカテゴリカル**へ
切り替えた。順は skill の推奨順（slot 1-6）で、**隣接ペアの分離**に最適化されている（ギヤは
隣り合う段が経路上でも隣接するのでこの条件が効く）。既知の弱点は 2速(橙) と 4速(黄) が
全ペア条件で通常視 ΔE 13.7 と floor を割ること。凡例に数字を併記して色だけに頼らせない形で
補い、`表` ボタンで数値表も開ける。

OSM タイルに直接乗せると明るい色がコントラスト floor を割るため、線には暗いケーシングを
敷いて隣接面をそちらにしている。

**判定できない場合は意味で 3 つに分ける。** 混ぜると読み違えるため。

| 状態 | 地図 | グラフ |
|---|---|---|
| ギヤ確定 | 6 色の実線 | 実線 |
| 半クラ・シフト（比は出たが窓の外） | 灰の細い破線 | **細い破線で繋ぐ** |
| 低速（比が定義できない） | 灰の細い点線 | 同上 |
| **停車**（判定する対象が無い） | 描かない | **ハッチングの帯** |

**グラフは欠測の区間を切らずに細い破線で繋ぐ。** ギヤは半クラ・シフトのたびに判定不能に
なるため、切ると線が細切れになって前後の段が読めない。破線＝データが無い区間、実線＝実測、
と読み分けられるようにしてある。繋ぎを実測より細くしているのは、同じ太さだと同じ重みに
見えてしまうため。

停車は「判定できない」のではなく「判定する対象が無い」ので線を引かない。地図上では動いて
いないため線に意味が無く、時系列側で帯として示すほうが滞在時間も読める。
