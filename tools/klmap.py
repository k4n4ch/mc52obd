...
KNOWN = {
    ...
    (0x11, 5): ("**RPM 下位**", "同上"),
    (0x11, 6): ("スロットル系", "空吹かしに同期。**スケール未確定**"),
    (0x11, 7): ("スロットル系", "空吹かしに同期。0→5。**スケール未確定**"),
    ...
    (0xD1, 8): ("**エンジン状態（ビットフィールド）**",
                "bit0 運転（常時 1）/ bit1 常時 0 / **bit3 は約 15km/h のしきい値** / "
                "bit2 未同定（同じ速度・回転で噴射が 654/1176 に分かれる）"),
    (0xD1, 9): ("未同定", "実走 3837 サンプルで 00 固定。**段数ではない**"),
